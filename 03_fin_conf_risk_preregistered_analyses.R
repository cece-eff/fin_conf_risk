...
##effects of financial knowledge on confidence:
lm.kwage.conf <- lm(z_confid_instit ~ z_new_instkw_score + z_age, data = rush1wave_mod)

summary(lm.kwage.conf)
confint(lm.kwage.conf, c('z_new_instkw_score', 'z_age'), 0.95)

#### Run regression above on each level of group
lm(z_confid_instit ~ z_new_instkw_score*z_age, 
